<template>
  <view class="container">
	...
	<view class="location-info">
	  <text class="info-label">当前经纬度：</text>
	  ...
	  <view class="form-item">
	          ...
	          <view class="input-container">
	            <picker 
	              ...
	              class="form-picker"
	            >
	              <view :class="['picker-text', !formData.species ? 'placeholder-text' : '']">
	                {{ formData.species || '请选择物种类型' }}
	              </view>
	            </picker>
	          </view>
	        </view>
...
import {uploadFileToMinio,addPostanimalArchival,queryGetAnimalArchivalById,updateAnimalArchival} from '@/services/animalArchival/animalArchival.js'
...
// 物种选择 TODO
// 新增：物种选项数组
const speciesOptions = ['猫咪', '狗狗', '鸟类', '兔子', '其他'];
// 新增：处理物种选择事件
const handleSpeciesChange = (e) => {
  const index = e.detail.value;
  formData.value.species = speciesOptions[index];
};
...
onMounted(async () => {
  // 1. 获取存储的ID（注意：存储的是字符串类型的ID）
  getCurrentLocation();
  const storedId = uni.getStorageSync("animalArchivalUpdated");
  console.log("srpredId = " + storedId);
  
  // 2. 严谨判断：ID存在且不为空字符串
  if (storedId && storedId !== "") {
    console.log("存在 animalArchivalUpdated，ID为：", storedId);
    
    try {
      // 3. 传递ID并等待异步接口返回（必须加await）
      const animalData = await queryGetAnimalArchivalById(storedId);
      
      // 4. 正确赋值（使用data，而非date）
      if (animalData) {
        formData.value = animalData.data;
      } else {
        console.log("未查询到对应动物档案");
      }
    } catch (error) {
      // 5. 捕获接口调用错误
      console.error("查询动物档案失败：", error);
      uni.showToast({ title: error.message, icon: "none" });
    } finally{
		uni.removeStorageSync("animalArchivalUpdated");
	}
  } else {
    console.log("animalArchivalUpdated 不存在或为空");
  }
});
...
const onSubmit = async() => {
	formData.value.latitude = currentLocation.value.latitude;
	formData.value.longitude = currentLocation.value.longitude;
  // 表单验证
  ...
  try {
    ...
    if(formData.value.stringId !== null && formData.value.stringId !== undefined){
        console.log("stringId",formData.value.stringId);
        // 2. 这里直接赋值，不要加 const
        result = await updateAnimalArchival(formData.value);
    } else {
        // 2. 这里直接赋值，不要加 const
        result = await addPostanimalArchival(formData.value);
    }
    
    // 现在代码走到这里，就能正常打印 result 啦！
...
//   if(formData.value.stringId !== null && formData.value.stringId !== undefined){
// 	  console.log("stringId",formData.value.stringId);
// 	  const result = await updateAnimalArchival(formData.value);
//   }else{
...
// }
  ...
  uni.showToast({
    ...
    success: () => {
      setTimeout(() => {
		// uni.$emit('refreshAnimalList', true); 
        onBack();
      }, 1500);
    }
  });
...
.placeholder-text {
  color: #ccc; /* 模拟原本 input placeholder 的颜色 */
}
/* ------------------------ */
</style>
